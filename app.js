...
async function begin(review=false){const pool=review?words.filter(w=>['unknown','unsure'].includes(status(w.id))):words.filter(w=>w.level===level);if(!pool.length){notice='復習する単語がまだありません。';render();return}session={ids:pick(pool,limit),index:0,review,direction,shown:false,hinted:false,results:[]};await saveSession();setView('practice')}
...
async function rate(choice){
  ...
  const final=session.hinted&&choice==='known'?'unsure':choice;
...
function renderPractice(){
  ...
  app.innerHTML=`<div class="quizhead"><h1>${session.review?'苦手語の復習':`LEVEL ${w.level}`}</h1><span class="muted">${session.index+1} / ${session.ids.length}</span></div>
    ...
    <div class="answerbar">${session.shown?`<div class="rating-label">思い出せましたか？</div><div class="ratings"><button data-rate="known">わかる</button><button data-rate="unsure">あやふや</button><button data-rate="unknown">わからない</button></div>`:
      `<div class="hint" aria-live="polite">${japaneseFirst&&session.hinted?mask:''}</div><div class="actions">${japaneseFirst?'<button class="secondary" id="hint">最初の1文字を見る</button>':''}<button class="primary" id="reveal">答えを見る</button></div>`}</div></section>
    <p class="note">${session.hinted?'ヒントを使った語は「わかる」を選んでも「あやふや」に記録します。':'答えを確認してから、思い出せたかを選んでください。'}</p>
