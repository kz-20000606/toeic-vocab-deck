...
let words=[],records={},customLists=[],activeListId=null,db,view='home',level=1,direction='ja-en',orderMode='sequential',cursors={},session=null,notice='',listLevel='1';
...
async function begin(review=false,block=0){
  ...
  const pool=review?allWords().filter(w=>['unknown','unsure'].includes(status(w.id))):orderMode==='random'?levelWords:levelWords.slice(block*10,(block+1)*10);
...
function renderHome(){
  const weak=allWords().filter(w=>['unknown','unsure'].includes(status(w.id))).length;
...
async function createCustomList(event){
  ...
  await saveCustomList(list);customLists.push(list);activeListId=list.id;renderCustom();
}
...
async function editCustomWord(event,id){
  ...
  entry.word=word.slice(0,120);entry.meaning=meaning.slice(0,300);await saveCustomList(list);renderCustom();
}
async function deleteCustomWord(id){
  ...
  await transaction('progress','readwrite',store=>store.delete(id));delete records[id];
...
async function deleteCustomList(){
  ...
  await transaction('progress','readwrite',store=>list.words.forEach(w=>store.delete(w.id)));
  list.words.forEach(w=>delete records[w.id]);
...
function renderCustom(message=''){
  ...
  app.innerHTML=`<div class="eyebrow">MY LISTS</div><h1 class="heading">自分の単語リスト</h1><p class="muted">自分だけのリストを作って、単語を追加できます。この端末のブラウザに保存されます。</p>
...
async function importData(e){
  ...
  try{
    ...
    const validIds=new Set(allWords().map(w=>w.id));const validStatus=new Set(['known','unsure','unknown']);
